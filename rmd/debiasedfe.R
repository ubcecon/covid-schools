...
multiple_split <- function(s, n, unit, time, dat, uc, fm) {
  ...
  for (k in 1:s) {
    ...
    cross3 <- felm(fm, data = dat,
                   weights = dat$sweight)
    across <- across + coef(cross3)/s 
  }
...
reg_fe <- function(df, # data
  ...
  set.seed(123458)
  if (multiple==TRUE) {
    coef_acbc <- multiple_split(s = 2, n, unit, time, df, coef(whole), fmla[[1]]) 
    return(list(reg = whole, nobc = coef_nobc, bc = coef_acbc)) 
  } else { 
    s <- 2
    across <- 0 * coef(whole)
    for (k in 1:s) {
    ...
    coef_cbc2 <- 2*coef(whole) - across
    return(list(reg = whole, nobc = coef_nobc, bc = coef_cbc2)) 
  }
...
data_rg <- function(data, mle) {
  ...
  data_b$fips <- kronecker(c(1:n), rep(1, t))
  data_b$date <- rep(seq(as.Date("2020-05-01"), as.Date("2020-11-25"), by = "day"), n)
...
data_wb <- function(data, mle) {
  ...
  # Exponential weights
  multipliers <- rexp(n)
  w <- multipliers/sum(multipliers) 
  # For each state, weight is the same for all dates
  # Add to the data and treat it as sampling weight
  if (length(data$sweight)==n*t) {
    weight <- rep(w, each = t)
    data$sweight <- weight
  } else {
    for (i in 1:n) {
      data$sweight[data$fips==as.vector(f[[i]])] <- w[i]
    }
  }
  return(data)
}
