...
Specifically, a simplified version of our model is 
$$
\Delta y_{it} := y_{it}-y_{it-7} = \sum_{m =\text{remote,\ hybrid,\ full}} \beta_{m}  p^m_{it} + \sum_{\tau=14,21,28} \beta_{y,\tau} y_{i,t-\tau} +\alpha_i +\zeta_t + \epsilon_{it},
$$
...
$$
 y_{it}  =  \sum_{m =\text{remote,  hybrid,  full}} \beta_{m}  p^m_{it} +   \sum_{\tau=7, 14,21,28} \beta_{y,\tau} y_{i,t-\tau} +\alpha_i +\zeta_t + \epsilon_{it}
$$
under the restriction of $\beta_{y,7}=1$. For robustness check, we estimate this alternative specification without restricting that $\beta_{y,7}=1$.


Given the referee's request, we estimate the above model for robustness check. Our view, however, is that controlling for past cases/deaths is imoprtant to achieve our panel regression assumption that the timing and the mode of school opening is random conditional on controlled confounders. 


```{r piy2, eval = FALSE, cache=FALSE, warning=FALSE, results='asis'}
 
# generate the log case with weekly lag
sdf$logdc_lag <- panellag(sdf$logdc,sdf$fips, sdf$date, lag=7)

# Mask requirement for staffs  
# infov <- c("logdc", "logdc_L", "logdc_L2")   
infov <- c("logdc_lag","logdc", "lag(logdc,7)", "lag(logdc,14)")   
sdf$week <- week(sdf$date) 
#interactions <-list("as.factor(month)", "as.factor(state)")  
interactions2 <-list("as.factor(week)", "as.factor(state)")    
infolist <- list(infov)
...
Given the referee's request, we also estimate the model without lagged dependent variables although we think it is important to control for these lagged dependent variables because they are obvious confounders once we realize that people voluntarily change their behavior beyond what we see in the data when the risk of virus transmission is high because the reported case number is high. 


```{r piy3, eval = FALSE, cache=FALSE, warning=FALSE, results='asis'}
...
plist <- list(pols1,pols2,pols3,pols4)   
cluster = "state" 
debias <- FALSE # No need to do debiasing because we don't have any lagged dependent variable
includepbiy <- FALSE
includepiy <- TRUE
includepib <- FALSE     
includeip <- FALSE
...
quantileplot <- function(v, p=c(0.25, 0.5, 0.75, 0.95, 0.99), data=df) {
  data$y <- data[,v]
  adf <- aggregate(y ~ date, data=data,
                   FUN=function(x) quantile(x, probs=p, na.rm=TRUE))
  q <- quantile(data$y, probs=c(0.001,0.999), na.rm=TRUE)
  p <- as.factor(p)
  s <- 1.2
  ...
  figm <- ggplot(adf) +
   ...
   # geom_line(data=data, aes(x=date, y=y, group=fips), alpha=0.002) +
    ...
    scale_x_date(limits = as.Date(c("2020-02-15","2020-12-02"))) 
}
...
quantileplot <- function(v, p=c(0.05,0.25, 0.5, 0.75, 0.95), data=df) {
  ...
  figm <- ggplot(adf) +
    ...
    #geom_line(data=data, aes(x=date, y=y, group=fips), alpha=0.002) +
    theme(legend.position="none") +
    scale_x_date(limits = as.Date(c("2020-03-20","2020-12-31")),date_breaks = "months" , date_labels = "%b")
}
...
df2 <- subset(df, sdf$PopulationEstimate2018>q)  
quantileplot2 <- function(v, p=c(0.05,0.25, 0.5, 0.75, 0.95), data=df2) {
  data$y <- data[,v]
  adf <- aggregate(y ~ date, data=data,
                   FUN=function(x) quantile(x, probs=p, na.rm=TRUE))
  q <- quantile(data$y, probs=c(0.001,0.999), na.rm=TRUE)
...
f <- function(i) {
  ...
  res <- cbind(m$reg$beta[1:10],m$bc[1:10],m$reg$se[1:10]) 
  return(res)
} 
...
dwfig_d <- function(ij,k,y,debias=TRUE) {
  ...
  results_df <- data.frame(term = rep(var.names,times = np),
                         estimate = c(coef[k,ij]),
                         std.error = c(se[k,ij]), 
                         model = c(rep("(1) Baseline",nv),  rep("(2) Lag = 28",nv),rep("(3) Lag = 35",nv),  
...
ij <- c(1:np) 
nv <- length(var.names)
fig <- dwfig_d(ij,k,piy_d,debias = FALSE) + xlab("Estimated Coefficients") + xlim(-0.25, 1.0) + theme(legend.justification=c(.01, .999), legend.position=c(.62, .99), legend.title = element_text(size=12, face='bold'), legend.text = element_text(size = 12), legend.background = element_rect(color="gray90"), axis.text.x = element_text(size = 12), axis.text.y = element_text(size = 12) ) 
p <- "school-college" 
figdev(sprintf("%s/tex/tables_and_figures/%s-whisker-death%s.pdf", rootdir, p, L), width=8, height=6) 
print(fig)
dev.off()
fig 
fig <- dwfig_d(ij,k,piy_d,debias = TRUE) + xlab("Estimated Coefficients") + xlim(-0.15, 1.2) + theme(legend.justification=c(.01, .999), legend.position=c(.62, .99), legend.title = element_text(size=12, face='bold'), legend.text = element_text(size = 12), legend.background = element_rect(color="gray90"), axis.text.x = element_text(size = 12), axis.text.y = element_text(size = 12) )
p <- "school-college-bc"
figdev(sprintf("%s/tex/tables_and_figures/%s-whisker-death%s.pdf", rootdir, p, L), width=8, height=6)
print(fig)
dev.off()
...
ij <- c(1:np) 
nv <- length(var.names)
fig <- dwfig_d2(ij,k,piy_d2,debias = FALSE) + xlab("Estimated Coefficients") + xlim(-0.25, 1.0) + theme(legend.justification=c(.01, .999), legend.position=c(.62, .99), legend.title = element_text(size=12, face='bold'), legend.text = element_text(size = 12), legend.background = element_rect(color="gray90"), axis.text.x = element_text(size = 12), axis.text.y = element_text(size = 12) ) 
p <- "school-college" 
figdev(sprintf("%s/tex/tables_and_figures/%s-whisker-death-hetero%s.pdf", rootdir, p, L), width=8, height=6) 
print(fig)
dev.off()
fig 
fig <- dwfig_d2(ij,k,piy_d2,debias = TRUE) + xlab("Estimated Coefficients") + xlim(-0.15, 1.2) + theme(legend.justification=c(.01, .999), legend.position=c(.62, .99), legend.title = element_text(size=12, face='bold'), legend.text = element_text(size = 12), legend.background = element_rect(color="gray90"), axis.text.x = element_text(size = 12), axis.text.y = element_text(size = 12) )
p <- "school-college-bc"
figdev(sprintf("%s/tex/tables_and_figures/%s-whisker-death-hetero%s.pdf", rootdir, p, L), width=8, height=6)
print(fig)
dev.off()
