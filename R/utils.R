...
smoothedpolicy <- function(df, pdatevar, enddatevar=NA, type="pnorm",
                           bw=7, lag=0, id="state") {
  # pseries have strange methods for >= and other operations
  class(df$date)  <- class(df$date)[!grepl("pseries",class(df$date))]
  class(df[,pdatevar])  <- class(df[,pdatevar])[!grepl("pseries",class(df[,pdatevar]))]
  if (type=="pnorm") {
    ...
    d[is.na(d)] <- FALSE
    if (!is.na(enddatevar)) {
      class(df[,enddatevar])  <- class(df[,enddatevar])[!grepl("pseries",df[,enddatevar])]
